...
import { cleanCollectionPath, encodeDoc, isDocumentPath } from './shared';

let db: admin.firestore.Firestore;
let batch: admin.firestore.WriteBatch;
let batchCount = 0;
let totalSetCount = 0;
let totalDelCount = 0;
let args;
...
export const execute = async (app: admin.app.App, file: string, collections: string[], options) => {
	db = app.firestore();
	batch = db.batch();
	...
	return new Promise(async (resolve, reject) => {
		...
		if (collections[0] === '/') {
			rootJsonCollections.forEach(coll => {
				const path = coll.substring(args.collPrefix.length + 1);
				data[path] = json[coll];
			})
			resolve(data);
			return;
