...
var db;
var args;
var execute = function (app, file, collectionPaths, options) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                ...
                return [4 /*yield*/, db.listCollections().then(function (colls) { return colls.map(function (coll) { return coll.path; }); })];
            case 1:
...
function getCollection(path) {
    ...
    return db.collection(path).get().then(function (snaps) { return __awaiter(_this, void 0, void 0, function () {
        var _i, _a, snap, doc, subCollPaths, subCollections;
        var _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    // try {
                    if (snaps.size === 0) {
                        throw "No ducuments in collection: ".concat(path);
                    }
                    _i = 0, _a = snaps.docs;
                    _c.label = 1;
                case 1:
                    ...
                    snap = _a[_i];
                    doc = (_b = {}, _b[snap.id] = snap.data(), _b);
                    // log if requested
                    args.verbose && console.log(snap.ref.path);
                    // Decode Doc
                    (0, shared_1.decodeDoc)(doc[snap.id]);
                    if (!args.subcolls) return [3 /*break*/, 4];
                    return [4 /*yield*/, snap.ref.listCollections().then(function (colls) { return colls.map(function (coll) { return coll.path; }); })];
                case 2:
        ...
        var collPath = "".concat(args.collPrefix, ":").concat(collId);
...
function bookWriteCSV(book, file) {
    ...
    // write index file
    var filename = __spreadArray([], fileParts, true);
...
function json2book(json) {
    ...
    var collectionIndex = [];
    book.Props = __assign(__assign({}, book.Props), { Title: 'FireStore Export', Author: 'firestore-migrator', CreatedDate: new Date() });
    var addCollection = function (coll, path) {
        var sheetName = (0, nanoid_1.nanoid)();
        var docs = [];
    ...
    collectionIndex.sort((0, shared_1.sortByKeysFn)(['depth', 'path']));
    collectionIndex.forEach(function (coll, index) {
        var n = index + 2;
        indexSheet['!ref'] = "A1:E".concat(n);
        indexSheet["A".concat(n)] = { t: 's', v: coll.sheetName };
        indexSheet["B".concat(n)] = { t: 's', v: coll.path };
        indexSheet["C".concat(n)] = { t: 'n', v: +coll.depth };
        indexSheet["D".concat(n)] = { t: 'n', v: +coll.count };
        indexSheet["E".concat(n)] = { t: 's', v: 'link', l: { Target: "#".concat(coll.sheetName, "!A1") } };
    });
